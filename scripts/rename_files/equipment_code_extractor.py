...
# Matches 2–3 letters, a dash, then 2 digits, e.g. MT-03 or FLD-12
_EQUIP_PATTERN = re.compile(r"\b[A-Z]{2,3}-\d{2}\b")

def extract_equipment_codes(blocks:List[str]) -> List[str]:
    """
    Given a list of title‑block strings, scan each for equipment codes
    matching YY-XX or YYY-XX, dedupe in order, and return them.
